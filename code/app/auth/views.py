...
from app.auth.forms import RegistrationForm, LoginForm, RequestResetPasswordForm, ResetPasswordForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        SpecialSym="*?!'^+%&/()=}][{$#"
        if len(password) < 8:
            flash('Password too short. Password must be 8 characters, have at least one upper case letter, one digit, and one special character','error')
            return redirect(url_for('auth.register'))
        if not any (char.isupper() for char in password):
            flash('No upper case letter found. Password must be 8 characters, have at least one upper case letter, one digit, and one special character','error')
            return redirect(url_for('auth.register'))
        if not any (char.isdigit() for char in password):
            flash('No digit found. Password must be 8 characters, have at least one upper case letter, one digit, and one special character','error')
            return redirect(url_for('auth.register'))
        if not any(char in SpecialSym for char in password):
            flash('No special character found. Password must be 8 characters, have at least one upper case letter, one digit, and one special character','error')
            return redirect(url_for('auth.register'))
